import { decryptt } from "../enDeCode/enDeCode";
...
const updateShareNoteService = async (uid:string, nid: string,  share: number) => {
    const check: Note | null = await noteModel.findOne({ where: { nid: nid, uid:uid } });
    if (check) {
...
const setImportanceNote = async (nid: string, uid:string) => {
    const check: Note | null = await noteModel.findOne({ where: { nid: nid, uid:uid  } });
    if (check) {
...
const getNoteByCodeService = async (code: string) => {
    const decode = decryptt(code)

    const check: Note | null = await noteModel.findOne({ where: { nid:decode} });
    if (check && check.share !==1) {
        return check;
    }
    else {
        return 404; // không tìm thấy note
